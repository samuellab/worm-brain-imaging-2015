function posture = posture_from_head(head_trajectory, xhat, L, varargin)
...
for t = 2 :size_T
    ...
    px = interp1(1:size(X,2), X(1,:), ix:-1:(ix-(N_seg-1)), ...
        'spline', 'extrap');
    px = smooth(px, round(0.25*N_seg));
    py = interp1(1:size(X,2), X(2,:), ix:-1:(ix-(N_seg-1)), ...
        'spline', 'extrap');
    py = smooth(py, round(0.25*N_seg));
    
    [px, py] = enforce_length(px, py, L);
