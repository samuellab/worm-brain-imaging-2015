function feature = register_local_max(parent, varargin)
% feature = REGISTER_LOCAL_MAX(parent, options)
%
%   Registers a PointFeature (parent) in an image with the specified 
%   optiosn. This method attempts to find a bright spot in the same
%   location as the parent feature.
%
...
for step = 1:options.steps
    ...
    local_max = find_subpixel_max(A);

    box_center = center_from_size(box_size);
    
    new_center = old_center + (local_max - box_center);
